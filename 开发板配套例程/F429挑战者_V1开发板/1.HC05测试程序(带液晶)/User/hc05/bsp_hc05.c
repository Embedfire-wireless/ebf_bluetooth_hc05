...
void printBLTInfo(BLTDev *bltDev)  
{
	...
	else
	{
		...
		sprintf(disp_buff," %d device found.",bltDev->num);
		
		LCD_SetColors(LCD_COLOR_RED,LCD_COLOR_BLACK);

		LCD_ClearLine(LCD_LINE_10);
		LCD_DisplayStringLine( LCD_LINE_10,(uint8_t *)disp_buff );
		...
		for(i=0;i<bltDev->num;i++)
		{
			...
			LCD_DisplayStringLine( LCD_LINE_12,(uint8_t *)disp_buff );

			
			sprintf(disp_buff,"Device Addr: %s",bltDev->unpraseAddr[i]);
			LCD_ClearLine(LCD_LINE_13);
			LCD_DisplayStringLine( LCD_LINE_13,(uint8_t *)disp_buff );
			
			sprintf(disp_buff,"Device name: %s",bltDev->name[i]);
			LCD_ClearLine(LCD_LINE_14);
			LCD_DisplayStringLine( LCD_LINE_14,(uint8_t *)disp_buff );
		}
	}
